...
newtype ML a = ML [a] deriving (Show, Eq, Monoid, Semigroup, Foldable, Functor, Traversable)

deriving instance ToJSON a => ToJSON (ML a)

instance FromJSON a => FromJSON (ML a) where
  parseJSON A.Null = pure $ ML []
  parseJSON v = ML <$> parseJSON v
...
data Endpoint
  ...
  | -- | Either pull an image from docker hub or imports an image from a tarball (or URL)
...
toContainerID t =
  if T.all (\c -> isAlphaNum c || c == ':') t -- Note: Can we improve this whitelist?
    then Just $ ContainerID t
    else Nothing
...
instance FromJSON StatusCode where
  parseJSON (JSON.Object o) = do
    c <- o .: "StatusCode"
    if c >= 0 && c <= 255
      then return (StatusCode c)
      else fail "Unknown exit code"
...
data NetworkSettings = NetworkSettings
  ...
  , networkSettingsSecondaryIPAddresses :: ML Text -- TODO: 1.24 spec is unclear
  , networkSettingsSecondaryIPv6Addresses :: ML Text -- TODO: 1.24 spec is unclear
...
instance ToJSON NetworkingConfig where
  toJSON (NetworkingConfig endpointsConfig) =
    JSON.object
...
defaultContainerConfig imageName =
  ContainerConfig
    { hostname = Nothing
    , domainname = Nothing
    , user = Nothing
...
defaultContainerResources =
  ContainerResources
    { cpuShares = Nothing
    , blkioWeight = Nothing
    , blkioWeightDevice = mempty
    , blkioDeviceReadBps = mempty
    , blkioDeviceWriteBps = mempty
    , blkioDeviceReadIOps = mempty
    , blkioDeviceWriteIOps = mempty
    , cpuPeriod = Nothing
    , cpusetCpus = Nothing
    , cpusetMems = Nothing
    , devices = mempty
...
data BuildOpts = BuildOpts
  ...
  , -- | Always attempt to pull a newer version of the *parent* image (ie. FROM debian:jessie).
    buildPullParent :: Bool
  }
  deriving (Eq, Show)

defaultBuildOpts :: Text -> BuildOpts
defaultBuildOpts nameTag =
  BuildOpts
    { buildImageName = nameTag
...
instance {-# OVERLAPPING #-} ToJSON [LogDriverOption] where
  ...
  toJSON (o : os) = toJsonKeyVal (o : os) key val
    where
      key (LogDriverOption n _) = T.unpack n
...
setCmd ccmd c = c {containerConfig = cc {cmd = ML [ccmd]}}
  where
...
addLink l c = c {hostConfig = hc {links = ols <> ML [l]}}
  where
    hc = hostConfig c
...
addVolumeFrom vf c = c {hostConfig = hc {volumesFrom = oldvfs <> ML [vf]}}
  where
    hc = hostConfig c
...
instance {-# OVERLAPPING #-} ToJSON [PortBinding] where
  ...
  toJSON (p : ps) = toJsonKeyVal (p : ps) key val
    where
      key p = T.unpack $ portAndType2Text (containerPort p) (portType p)
      val p = hostPorts p
...
instance ToJSON HostConfig where
  toJSON HostConfig {..} =
    let arr =
     ...
     in object $ arr <> resourcesArr resources
    where
...
data ContainerResources = ContainerResources
  ...
  , blkioWeightDevice :: ML DeviceWeight
  , blkioDeviceReadBps :: ML DeviceRate
  , blkioDeviceWriteBps :: ML DeviceRate
  , blkioDeviceReadIOps :: ML DeviceRate
  , blkioDeviceWriteIOps :: ML DeviceRate
...
instance {-# OVERLAPPING #-} ToJSON [ExposedPort] where
  ...
  toJSON (p : ps) = toJsonKey (p : ps) key
    where
      key (ExposedPort p t) = show p <> slash <> show t
      slash = T.unpack "/"
...
toJsonKey vs getKey = JSON.Object $ foldl f HM.empty vs
  where
    f acc x = HM.insert (T.pack $ getKey x) (JSON.Object HM.empty) acc
...
toJsonKeyVal vs getKey getVal = JSON.Object $ foldl f HM.empty vs
  where
    f acc x = HM.insert (T.pack $ getKey x) (toJSON $ getVal x) acc
