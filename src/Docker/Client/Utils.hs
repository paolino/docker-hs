...
--
import Docker.Client.Http (DockerError (DockerClientError))
...
makeBuildContext base =
  liftIO $
    tryIOError (makeBuildContext' base) >>= \case
...
parseDockerIgnoreFile c =
  DockerIgnore
    { exclusionPatterns = parseExclusions
    , inclusionPatterns = parseInclusions
    }
  where
    lines = filter (not . T.isPrefixOf "#") (T.lines c) -- Ignore comments
...
shouldInclude d = check `fmap` fileName
  where
    check f = dockerIgnoreDecision (exclusionCheck f (exclusionPatterns d), inclusionCheck f (inclusionPatterns d))

shouldRecurse :: DockerIgnore -> RecursionPredicate
shouldRecurse d = check `fmap` fileName
  where
    check f = dockerIgnoreDecision (exclusionCheck f (exclusionPatterns d), inclusionCheck f (inclusionPatterns d))
...
exclusionCheck f = any (\(ExclusionPattern p) -> f ~~ T.unpack p)

inclusionCheck :: FilePath -> [InclusionPattern] -> Bool
inclusionCheck f = any (\(InclusionPattern p) -> f ~~ T.unpack p)
